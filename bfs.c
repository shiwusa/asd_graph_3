...
void risovac ( HDC hdc, double** A, int* nx, int* ny )
{
            ...
            if( checker == 0 ) TextOut(hdc,100,100,"BFS graph",9);
            for (int i = 0; i < 11; i++)
            {
                for (int j = 0; j < 11; j++)
                {
                            ...
                            //printf("\n%i\t%i\n",i,j);
                                ...
                                if ( A[j][i] == 1 ) {
                                    ...
                                    if ( nx[i] == nx[j] ) {
                                        ...
                                        if( ny[i] - ny[j] == 6*oneLen ) arrow(100,nx[j]+3,ny[j]+dy);
                                        else arrow(-100,nx[j]+3,ny[j]-dy);
                                    }
                                    else if ( nx[i]-2*oneLen == nx[j] ) {
                                        arrow(130,nx[j]+6,ny[j]+dy);
                                    }
...
DWORD WINAPI somethingcool (LPVOID lpParam)
{
    ...
    printf("\n\nBuild bfs graph with new numbers");
    zhdac(lpParam);
    numeCheck = 1;
    printf("\n\nHere it is:");
    checkanddraw(lpParam);
...
void dfcheck ( HWND hWnd, int calledFor )
{
    ...
    int pos = 0;
    int que[11] = {};
    for ( int i = 0; i < 11; i++ ) que[i] = -1;
    que[pos] = calledFor;
    visited[calledFor] = 1;

    int n;
    int numb = 0;
    while ( numb < 11 && que[numb] != -1)
    {
        n = que[numb];
        numb++;
